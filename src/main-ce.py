...
from data import multiLabelDataset, make_tokenizer, collate_fn_feature_mix
...
class Recoder_multi():
    def __init__(self,args):
        ...
        self.acc = []
        self.step = []
    def log_train(self,ce_loss_x, loss):
...
while(step < args.steps):
    ...
    for batch in train_loader:
        ...
        # print(batch)
        if args.feature_mix:
          ce_loss_x, ce_loss_s = model.forward_feature_mix_ce(batch)
          ce_loss = (ce_loss_x + ce_loss_s)/2
        else:
            ce_loss_x = model.forward_corssentropy_alone(batch)
        ce_loss = ce_loss_x
        ...
        if (count % args.num_accum == 0):
            optimizer.step()
            recoder.log_train(ce_loss_x ,loss)
            step += 1
            optimizer.zero_grad()
