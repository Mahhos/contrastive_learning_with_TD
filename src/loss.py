...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        print("11111111", features.shape)
        batch_size = features.shape[0] ## 2*N
        print("333333333", batch_size)
        print("22222222", mask)
        if labels is not None and mask is not None:
            ...
            print("4444444444", labels)

            labels = labels.contiguous().view(-1, 1)

            if labels.shape[0] != batch_size:
                raise ValueError('Num of labels does not match num of features')
            print("555555555", labels, labels.T)

            mask = torch.eq(labels, labels.T).float().to(device)
            print("555555555", mask.shape, mask)
        ...
        print("66666666666", logits.shape, logits_mask.shape)
