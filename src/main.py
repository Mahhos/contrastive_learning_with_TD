...
from data import collate_fn, collate_fn_mix, multiLabelDataset, multiLabelTwoDataset, make_tokenizer,collate_fn_feature_mix, collate_fn_feature_mix_cartography
...
class TestableIO(io.BytesIO):
    ...
    def write(self, bytes):
        if '222222222' in bytes or '33333333333' in bytes:
...
class Recoder_multi():
    ...
    def log_train(self,ce_loss_x, ce_loss_s, scl_loss, loss):
        ...
        #print('8888888888888888888', type(scl_loss))
    ...
    def meter(self,step):
        ...
        with open('recorder.txt', 'a') as f:
            ...
            f.writelines('\n')
            f.writelines("loss: "+ str(sum(self.loss[st:ed])/self.args.log_step))
            f.writelines('\n')
            f.writelines("ce_loss_x: " + str(sum(self.ce_loss_x[st:ed])/self.args.log_step))
            f.writelines('\n')
            f.writelines("ce_loss_s: " + str(sum(self.ce_loss_s[st:ed])/self.args.log_step))
            f.writelines('\n')
            f.writelines("scl_loss: " + str(sum(self.scl_loss[st:ed])/self.args.log_step))
            f.writelines('\n')
...
#parser.add_argument("--model",type=str,default="xlnet") # mahshid
parser.add_argument("--model",type=str,default="roberta")
parser.add_argument("--task",type=str,default=None)
parser.add_argument("--cartography", action='store_true')
parser.add_argument("--d2", type=str,default="ambig")
...
# mahshid
if args.cartography:
    train_dataset = multiLabelTwoDataset(dataset_name = args.dataset, d2=args.d2 ,max_num=args.train_num,seed=args.seed,split="train",aug_methods = args.aug_methods)
    test_dataset = multiLabelTwoDataset(dataset_name = args.dataset,d2=args.d2 , max_num=1000,seed=args.seed,split="test",aug_methods = args.aug_methods)
else:
...
# mahshid
if args.cartography:
  my_collect = collate_fn_feature_mix_cartography

print('my_collect', my_collect)
...
log_name = "Seed-" + str(args.seed) + "_" + args.model + "_" + args.dataset + "_"
if args.with_mix:
  log_name += "with_mix_"
...
if args.cartography:
    log_name += "cartography_easy_ambig_"
if args.feature_mix:
...
while(step < args.steps):
    ...
    for x,x_perm,s,s_perm,y_a,y_b in train_loader:
        ...
        if args.feature_mix:
          ce_loss_x, ce_loss_s, scl_loss = model.forward_feature_mix_new(batch) # forward_feature_mix_new or forward_feature_mix
        else:
        ...
        if begin_eval:
            ...
            #torch.save(recoder, "../" + 'Seed-2_roberta_csn_with_mix_with_summary_summary80_feature_mix_0.9_easy_ambig.pkl')
